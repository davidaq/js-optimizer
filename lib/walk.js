/**
 * Walk through an already traced ast,
 * can replace nodes by returning a new replacement
 * in the callback function
 */

var trace = require('./trace');

module.exports = walk;

function walk(ast, pre, post) {
    if (pre && typeof pre !== 'function') {
        throw new Exception('Pre-callback must be a function');
    }
    if (post && typeof post !== 'function') {
        throw new Exception('Post-callback must be a function');
    }
    var stack = [ast];
    var ret = ast;
    while (stack[0]) {
        var ele = stack.pop();
        if (!ele)
            break;
        if (post && ele.type == 'StackPop') {
            replace(ele.node, post(ele.node));
        } else {
            if (pre) {
                var nele = pre(ele);
                if(replace(ele, nele)) {
                    ele = nele;
                }
            }
            if(ele === null || typeof ele !== 'object') continue;
            if (post) {
                stack.push({
                    type: 'StackPop',
                    node: ele
                });
            }
            if (ele.children) {
                for (var i = ele.children.length; i--;) {
                    stack.push(ele.children[i]);
                }
            }
        }
    }
    function replace(ele, replace) {
        if (typeof replace == 'undefined') {
            return;
        }
        if (typeof replace === 'string') {
            replace = {
                type: 'raw',
                content: replace
            };
        }
        if (typeof replace !== 'object') {
            replace = null;
        }
        if (ele.parent) {
            var d = ele.parent.breadcrumb.length;
            d -= ele.breadcrumb.length;
            var k = ele.breadcrumb.slice(d);
            if (k.length > 0) {
                var o = ele.parent;
                while (k.length > 1) {
                    o = o[k.shift()];
                }
                if (replace) {
                    o[k[0]] = replace;
                } else if(o instanceof Array) {
                    o.splice(k[0], 1);
                } else {
                    o[k[0]] = null;
                }
            }
            trace(ele.parent);
        } else {
            trace(replace);
            ret = replace;
        }
        return true;
    }
    return ret;
}

